...
fetching_options = dict((key, ','.join(value))
                        for key, value in fetching_options.items())
...
def fetch_data():
    for file in dataset_file:
        ...
        dataset_df = dataset_df.assign(tweet_ids=dataset_df['tweet_ids'].str.split('\t')).explode(
            'tweet_ids').reset_index()
            ...
            while not complete:
                try:
                    ...
                    tweets = client.get_tweets(
                        ids=ids, expansions=fetching_options['expansions'], tweet_fields=fetching_options['tweet_fields'], user_fields=fetching_options['user_fields'])
                    ...
                    error_file.write(json.dumps(
                        [*error_file_data_json, *error_data]))

                    # Tweets Data
                    tweets_data = list(map(lambda x: x.data, tweets.data))
                    for tweet in tweets_data:
                        tweet_file = open(
                            f'{data_dir}/tweets/{tweet["id"]}.txt', 'w')
                        tweet_file.write(json.dumps(tweet))
                        tweet_file.close()

                    # User Data
                    users_data = list(
                        map(lambda x: x.data, tweets.includes['users']))
                    for user in users_data:
                        user_file = open(
                            f'{data_dir}/users/{user["id"]}.txt', 'w')
